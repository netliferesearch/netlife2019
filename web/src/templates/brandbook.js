  ...
  return (
    <>
      ...
      <Layout breadcrumb={pageContext.breadcrumb}>
        ...
        {intro && <div className="mt-16 text-md md:w-2/3">{intro}</div>}
        <section className="flex flex-wrap">
          <div className="w-full md:w-1/3 mt-16">
            {typeof brandbookPages === 'object' && (
              <>
                <h2 className="text-md mb-2">Merkevaren</h2>
                <ul>
                  {brandbookPages.map((page, i) => (
                    <li
                      key={`brandbook-page-${page.title}`}
                      className={`md:mr-12 py-2 ${i + 1 !==
                        brandbookPages.length && 'border-b border-black'}`}
                    >
                      {page?.slug?.current ? (
                        <Link
          ...
          <div className="w-full md:w-1/3 mt-16">
            {typeof brandbookPages === 'object' && (
              <>
                <h2 className="text-md mb-2">Maler</h2>
                <ul>
                  {brandbookTemplates.map((template, i) => (
                    <li
                      key={`brandbook-template-${template.title}`}
                      className={`md:mr-12 py-2 ${i + 1 !==
                        brandbookTemplates.length && 'border-b border-black'}`}
                    >
                      {template?.slug?.current ? (
                        <Link
                          href={template.slug.current}
          ...
          {typeof brandbookContent === 'object' &&
            brandbookContent.map((block, i) => (
              <div
                key={`brandbook-block-${block?.linkToBrandbookPage?.title ||
                  i}`}
                className="w-full md:w-1/3 mt-16"
              >
                <div className="md:mr-12">
                  <h2 className="text-md font-lining">
                    {block?.linkToBrandbookPage?.slug?.current ? (
                      <Link
                        href={block.linkToBrandbookPage.slug.current}
                        title={block.linkToBrandbookPage.title}
                        className="link"
                      >
                  ...
                  {block?.linkToBrandbookPage?.intro && (
                    <p className="mt-2">{block.linkToBrandbookPage.intro}</p>
                  )}
                  {block?.image?.image && (
                    <div className="mt-2">
                      <Image
