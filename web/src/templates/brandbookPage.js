...
export default ({ data, pageContext, location }) => {
  ...
  const pageContactTitle = contactSection && contactSection?.title ? contactSection?.title : null;
  const pageContactIntro = contactSection && contactSection?.contactIntro ? contactSection?.contactIntro : null;
  const pageContactPersons = contactSection && contactSection?.persons ? contactSection?.persons : null;
  ...
  const scrollTo = (toId) => {
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      window.scrollTo({top: document.getElementById(toId).offsetTop, behavior: 'smooth'});
    }
  };
      ...
      <Layout breadcrumb={pageContext.breadcrumb}>
        <section className="md:border-b border-solid border-black border-0 md:mb-16 md:pb-16">
          ...
          <div className="flex flex-wrap">
            {intro && <div className="text-lg w-full md:w-1/2">{intro}</div>}
            <ul className="mt-4 mb-8 md:my-0 w-full md:w-1/3 ml-auto">
              ...
              {sections && sections.map((section, i) => (
                <li key={section._key} className={`py-1${i < sections.length -1 && ' border-b border-solid border-black border-0'}`}>
                  <button
                    type="button"
                    className="link"
        ...
        {sections && sections.map((section, i) => (
          <section key={section._key} className={`${i < sections.length -1 && 'mb-8 md:mb-16 md:pb-16 border-b border-solid border-black border-0'}`}>
            ...
            {section?.content && section.content.map(c => {
              ...
              if (c._type === 'textImage') {
                ...
                return (
                  <div key={c._key} className="mb-8 md:mb-16 py-4 px-4" style={{backgroundColor: '#' + c.color}}>
                    {c.title && <h3 className="text-base">{c.title}</h3>}
                    {c.text && <h4 className="text-lg pt-8 pb-4 px-4">{c.text}</h4>}
                  </div>
                );
                ...
                return (
                  <div className="flex flex-wrap" key={c._key}>
                    {typeof c.blocks  === 'object' && c.blocks.map(block => (
                      <div key={block._key} className={`w-full md:w-${c.blocks.length > 2 ? '1/3' : '1/2'}`}>
                        <div className="md:mx-4 mb-8 md:mb-16 py-4 px-4" style={{backgroundColor: '#' + block.color}}>
                          {block.title && <h3 className="text-base">{block.title}</h3>}
                          {block.text && <h4 className="text-lg pt-8 pb-4 px-4">{block.text}</h4>}
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="pt-16 border-solid border-black border-t">
          <ContactSection heading={pageContactTitle || defaultHeading} intro={pageContactIntro} persons={pageContactPersons || defaultPersons} form={form} />
        </div>
      </Layout>
    </>
  );
